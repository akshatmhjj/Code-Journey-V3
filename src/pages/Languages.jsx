  ...
  numberFlow()
    ...
    .collect { println(it) }
      ...
      },
      {
        ...
        level: "Advanced",
        time: "4–6 weeks",
        tagline: "Build native iOS apps",
        about: "Swift is Apple’s programming language for building iOS, macOS, watchOS and iPadOS apps. It is fast, safe and modern — designed to replace Objective-C. Combined with SwiftUI, it allows developers to build clean, reactive interfaces with minimal code.",
        topics: [
          {
            name: "Swift Basics",
            icon: Code2,
            desc: "Variables (let/var), optionals, control flow (if, switch), and functions — the core building blocks of Swift."
          },
          {
            name: "Optionals",
            icon: Shield,
            desc: "Handling absence of values safely — ?, !, optional binding (if let) and avoiding crashes."
          },
          {
            name: "Structs & Classes",
            icon: Layers,
            desc: "Understanding value vs reference types — a key concept in Swift programming."
          },
          {
            name: "SwiftUI",
            icon: Paintbrush,
            desc: "Apple’s modern UI framework — building interfaces declaratively with views and state."
          },
          {
            name: "Async & Networking",
            icon: Server,
            desc: "Fetching data using async/await and URLSession — connecting apps to APIs."
          }
        ],
        snippet: `// 1. Variables
let name = "Alex"
...
/* Level badge */
const LevelBadge = ({ level, T, isMobile }) => {
  const cfg = { Foundation: { color: "#22c55e" }, Core: { color: "#7c6ee0" }, Advanced: { color: "#f97316" } };
  const c = cfg[level]?.color || T.t3;
  return (
    <span style={{
      fontFamily: "'JetBrains Mono',monospace", fontSize: isMobile ? 8 : 9.5, padding: isMobile ? "1px 5px" : "2px 6px", fontWeight: 700, borderRadius: 4, letterSpacing: "0.5px", background: `${c}16`, color: c, border: `1px solid ${c}30`
    }}>{level}</span>
  );
};
...
const LangPanel = ({ lang, trackColor, T, onClose, isMobile }) => (
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14, flexWrap: "wrap" }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
              ...
              <LevelBadge level={lang.level} T={T} isMobile={isMobile} />            </div>
      ...
      >
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {lang.topics.map((t, i) => (
              <TopicCard key={i} topic={t} color={lang.color} T={T} />
            ))}
          </div>
        </div>
...
const LangCard = ({ lang, T, onClick, isActive, isMobile }) => {
  const [hov, setHov] = useState(false);
  return (
    <motion.div layout onClick={onClick}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        <LevelBadge level={lang.level} T={T} isMobile={isMobile} />        <div style={{ display: "flex", alignItems: "center", gap: 4, marginLeft: "auto" }}>
                  ...
                  track.languages.map((lang, li) => (
                    <div key={lang.id}
                      style={{
                        ...
                        background: li % 2 === 0 ? T.card : T.panel,
                        borderBottom: `1px solid ${T.b1}`,
                        alignItems: "center",
                        gap: isMobile ? 2 : 4,
                        cursor: "pointer"
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                        <div style={{
                          borderRadius: 7, background: lang.bg, border: `1px solid ${lang.color}44`, display: "flex", alignItems: "center", justifyContent: "center", width: isMobile ? 22 : 26, height: isMobile ? 22 : 26, fontSize: isMobile ? 9 : 11, fontWeight: 700, color: lang.color, fontFamily: "'JetBrains Mono',monospace", flexShrink: 0
                        }}>{lang.icon}</div>
                        <span style={{ fontFamily: "'Syne',sans-serif", fontWeight: 600, fontSize: isMobile ? 11.5 : 13.5, color: T.t1 }}>{lang.name}</span>
                      </div>
                      {/* Track */}
                      < div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                        <track.icon size={12} color={track.color} />
                        <span style={{ fontFamily: "'Syne',sans-serif", fontSize: isMobile ? 10 : 12, color: T.t2 }}>{track.id === "web" ? "Web" : track.id === "app" ? "App" : "Data"}</span>
                      </div>
                      {/* Level */}
                      <LevelBadge level={lang.level} T={T} isMobile={isMobile} />                      {/* Time */}
                      <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: isMobile ? 10 : 11.5, color: T.t3 }}>{lang.time}</span>
                      {/* Tagline */}
                      <span style={{
                        fontFamily: "'Lora',serif", fontSize: isMobile ? 10.5 : 13,
                        whiteSpace: "nowrap",
                        overflow: "hidden",
                        textOverflow: "ellipsis", color: T.t2, fontStyle: "italic"
                      }}>{lang.tagline}</span>
                    </div>
                  ))
                )}
          ...
          <Reveal>
            <div style={{ maxWidth: 560, margin: "0 auto", textAlign: "center", position: "relative" }}>
              ...
              <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
                ...
                <Link to="/about" style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "13px 22px", borderRadius: 12, border: `1px solid ${T.b2}`, background: "transparent", color: T.t1, fontFamily: "'Syne',sans-serif", fontWeight: 600, fontSize: 15, cursor: "pointer", textDecoration: "none", transition: "all 0.18s" }}
